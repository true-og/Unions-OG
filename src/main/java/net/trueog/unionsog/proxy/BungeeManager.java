...
public final class BungeeManager implements ProxyManager, PluginMessageListener {
    ...
    private static final Pattern SUBCHANNEL_PATTERN = Pattern
            .compile("UnionsOG\\|(?<subchannel>\\w+)\\|(?<version>v\\d+)\\|(?<server>.+)");
    ...
    public void onPluginMessageReceived(@NotNull String channel, @NotNull Player player, byte[] data) {
        ...
        UnionsOG.debug("Message received, sub-channel: " + subChannel);
        try {
            ...
            Class<?> clazz = Class.forName("net.trueog.unionsog.proxy.listeners." + className);
            ...
            if (serverName != null && !isServerAllowed(serverName)) {
                ...
                UnionsOG.debug(String.format("Server not allowed: %s", serverName));
                return;
            ...
            UnionsOG.debug(String.format("Unknown channel: %s", subChannel));
            unsupportedChannels.add(subChannel);
    ...
    private String createSubchannelName(String subchannel) {
        ...
        return String.format("UnionsOG|%s|%s|%s", subchannel, VERSION, serverName);
    ...
    private void forwardPluginMessage(final String subChannel, final ByteArrayDataOutput message, final boolean all) {
        ...
        UnionsOG.debug(String.format("Forwarding message, channel %s, message %s, all %s", subChannel, message, all));
        if (!isChannelRegistered()) {
    ...
    private boolean isChannelRegistered() {
        ...
        UnionsOG.debug(String.format("BungeeCord channel registered: %s", registered));
        return registered;
