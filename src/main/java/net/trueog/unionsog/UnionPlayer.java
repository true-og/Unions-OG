...
import net.trueog.unionsog.managers.ProtectionManager.Action;
 ...
 * @author phaed
 */
public class UnionPlayer implements Serializable, Comparable<UnionPlayer> {
    ...
    private String tag;
    private @Nullable Union union;
    private boolean friendlyFire;
    private final Map<Kill.Type, Integer> kills = new HashMap<>();
    private int deaths;
    private long lastSeen;
    private long joinDate;
    private final Set<String> pastUnions = new LinkedHashSet<>();
    ...
    private boolean unionChatMute = false;
     ...
     */
    @Deprecated
    public UnionPlayer(String playerName) {
     ...
     */
    public UnionPlayer(UUID uuid) {

        uniqueId = uuid;
        Player onlinePlayer = UnionsOG.getInstance().getServer().getPlayer(uuid);
        if (onlinePlayer != null) {
            ...
            OfflinePlayer offlinePlayer = UnionsOG.getInstance().getServer().getOfflinePlayer(uuid);
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof UnionPlayer)) {

            return false;

        }

        UnionPlayer other = (UnionPlayer) obj;
        return other.getName().equals(getName());

    }

    @Override
    public int compareTo(UnionPlayer other) {
    ...
    public boolean isAlly(Player player) {
        ...
        UnionPlayer allycp = UnionsOG.getInstance().getUnionManager().getUnionPlayer(player);

        if (allycp != null) {

            // noinspection ConstantConditions
            return allycp.getUnion().isAlly(tag);
    ...
    public boolean isRival(Player player) {
        ...
        if (allycp != null) {
            ...
            // noinspection ConstantConditions
            return allycp.getUnion().isRival(tag);
    ...
    /**
     * Adds one rival kill to this player (does not update unionplayer to db)
     */
    @Deprecated
    ...
    /**
     * Adds one civilian kill to this player (does not update unionplayer to db)
     */
    @Deprecated
    ...
    /**
     * Adds one death to this player (does not update unionplayer to db)
     */
    ...
    /**
     * Returns the player's join date to his current union in milliseconds, 0 if not
     * in a union
     *
     * @return the joinDate
     */
    ...
    /**
     * Returns a string representation of the join date, blank if not in a union
     */
     ...
     * (used internally)
     *
     * @return the PackedPastUnions
     */
    public String getPackedPastUnions() {

        return String.join("|", getPastUnions());
     ...
     * (used internally)
     *
     * @param packedPastUnions the packedPastUnions to set
     */
    public void setPackedPastUnions(String packedPastUnions) {

        String[] unions = packedPastUnions.split("\\|");
        Collections.addAll(pastUnions, unions);

    }

    /**
     * Adds a past union to the player (does not update the unionplayer to db)
     */
    public void addPastUnion(String tag) {

        getPastUnions().add(tag);

    }

    /**
     * Removes a past union from the player (does not update the unionplayer to db)
     *
     * @param tag is the union's colored tag
     */
    public void removePastUnion(String tag) {

        getPastUnions().remove(tag);

    }

    /**
     * @param sep the separator
     * @return a separator delimited string with the color tags for all past unions
     *         this player has been in
     */
    @NotNull
    public String getPastUnionsString(@NotNull String sep) {

        return getPastUnionsString(sep, null);

    }

    /**
     * @param sep    the separator
     * @param viewer the Player viewing the union's string
     * @return a separator delimited string with the color tags for all past unions
     *         this player has been in
     */
    @NotNull
    public String getPastUnionsString(String sep, @Nullable Player viewer) {

        if (getPastUnions().isEmpty()) {
        ...
        SettingsManager settings = UnionsOG.getInstance().getSettingsManager();
        return getPastUnions().stream().sorted(Collections.reverseOrder()).limit(settings.getInt(PAST_UNIONS_LIMIT))
                .collect(Collectors.joining(sep));

    }

    /**
     * Returns a list with all past unions color tags this player has been in
     *
     * @return the pastUnions
     */
    public Set<String> getPastUnions() {
    ...
    /**
     * Returns a map containing the time the player resigned from certain unions
     *
    ...
    /**
     * Returns the time in millis when the player resigned from the union
     *
    ...
    public void setResignTimes(@Nullable Map<String, Long> resignTimes) {
        ...
        if (resignTimes != null) {
            ...
            final int cooldown = UnionsOG.getInstance().getSettingsManager().getInt(REJOIN_COOLDOWN);
    ...
    /**
     * Adds the union to the resign times map
     */
     ...
     */
    public void setUnion(@Nullable Union union) {
    ...
    /**
     * Returns this player's union's tag. Empty string if he's not in a union.
     *
     * @return the tag
     */
    @Placeholder("tag")
    ...
    /**
     * Returns this player's union's tag label. Empty string if he's not in a union.
     *
     * @return the tag
     */
    ...
    /**
     * Sets this player's trusted status (does not update the unionplayer to db)
     *
    ...
    @Deprecated
    public void setUnionChat(boolean unionChat) {
    ...
    public void setBbEnabled(boolean bbEnabled) {
        ...
        UnionsOG.getInstance().getStorageManager().updateUnionPlayer(this);
    ...
    public void setTagEnabled(boolean tagEnabled) {
        ...
        UnionsOG.getInstance().getStorageManager().updateUnionPlayer(this);
        UnionsOG.getInstance().getUnionManager().updateDisplayName(toPlayer());
    ...
    /**
     * @deprecated use {@link UnionPlayer#mute(Channel, boolean)}
     */
    @Deprecated
