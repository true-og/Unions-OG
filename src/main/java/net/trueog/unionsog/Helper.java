...
import static net.trueog.unionsog.utils.ChatUtils.stripColors;
...
public final class Helper {
    ...
    public static Locale forLanguageTag(@Nullable String languageTag) {
        ...
        Locale defaultLanguage = UnionsOG.getInstance().getSettingsManager().getLanguage();
        if (languageTag == null) {
    ...
    public static Set<Path> getPathsIn(String path, Predicate<? super Path> filter) {
        ...
        try {
            ...
            URI uri = UnionsOG.class.getProtectionDomain().getCodeSource().getLocation().toURI();
            ...
            UnionsOG.getInstance().getLogger().log(Level.WARNING,
    ...
    public static String getColorName(String playerName) {
        ...
        List<Player> players = UnionsOG.getInstance().getServer().matchPlayer(playerName);
