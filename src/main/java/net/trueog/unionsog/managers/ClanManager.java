...
import net.trueog.unionsog.events.CreateClanEvent;
...
public final class ClanManager {
    ...
    public void createClan(Player player, String colorTag, String name) {
        ...
        UnionsOG.getInstance().getPermissionsManager().updateClanPermissions(clan);
        UnionsOG.getInstance().getServer().getPluginManager().callEvent(new CreateClanEvent(clan));
    ...
    public boolean purchaseCreation(Player player) {
        ...
        if (plugin.getPermissionsManager().hasEconomy()) {
            ...
            if (plugin.getPermissionsManager().chargePlayer(player, price, Cause.CLAN_CREATION)) {
    ...
    public boolean purchaseInvite(Player player) {
        ...
        if (plugin.getPermissionsManager().hasEconomy()) {
            ...
            if (plugin.getPermissionsManager().chargePlayer(player, price, Cause.CLAN_INVITATION)) {
    ...
    public boolean purchaseHomeTeleport(Player player) {
        ...
        if (plugin.getPermissionsManager().hasEconomy()) {
            ...
            if (plugin.getPermissionsManager().chargePlayer(player, price, Cause.CLAN_HOME_TELEPORT)) {
    ...
    public boolean purchaseHomeTeleportSet(Player player) {
        ...
        if (plugin.getPermissionsManager().hasEconomy()) {
            ...
            if (plugin.getPermissionsManager().chargePlayer(player, price, Cause.CLAN_HOME_TELEPORT_SET)) {
    ...
    public boolean purchaseHomeRegroup(Player player) {
        ...
        if (!plugin.getSettingsManager().is(ECONOMY_UNIQUE_TAX_ON_REGROUP)) {
            ...
            shards = shards * VanishUtils.getNonVanished(player, clan).size();
                ...
                && plugin.getPermissionsManager().hasEconomy())
        {
            ...
            if (plugin.getPermissionsManager().chargePlayerShards(player, shards, Cause.CLAN_REGROUP)) {
            ...
            switch (clan.withdraw(operator, ClanBalanceUpdateEvent.Cause.COMMAND, shards)) {

                case SUCCESS:
                    if (plugin.getPermissionsManager().grantPlayerShards(player, shards, null)) {

                        player.sendMessage(AQUA
                                + lang("player.clan.withdraw", player, CurrencyFormat.formatShards(shards, player)));
                        clan.addBb(player.getName(), lang("bb.clan.withdraw", CurrencyFormat.formatShards(shards)));
                        return true;

                    }

                    clan.setBalance(operator, ClanBalanceUpdateEvent.Cause.REVERT, BankLogger.Operation.WITHDRAW,
                            clan.getBalance() + shards);
                    return false;
